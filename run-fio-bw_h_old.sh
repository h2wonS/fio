    ...
    do
        ...
        if (( ${base} == ${target} ))
            then
                ...
                echo "" | tee -a l result_$filename/latency_${filename}_99p_randread.dat
                continue
        fi
        ...
        cat  result_$filename/raw_${filename}_randread.dat | awk 'match($2, /99/) {print $0}' | awk 'NR==1{print substr($3,1,3)}' | tee -a  result_$filename/latency_${filename}_99p_randread.dat

    done
    echo "                                    " | tee -a  result_$filename/bw_${filename}_raw_randread.dat
    echo "                                    " | tee -a  result_$filename/latency_${filename}_99p_randread.dat
